...
def read_msa(
  ...
  Returns:
    A tuple of the form :obj:`(names, seqs)`

    - :obj:`names`: list of protein names from the a3m file, including gaps
    - :obj:`seqs`: list of protein sequences from the a3m file, including gaps
...
def align_mafft(seqs: Union[str, List[str], Dict[str, str]], ep: float = 0.0, op: float = 1.53) -> Tuple[List[str], List[str]]:
  ...
  Returns:
    A tuple of the form :obj:`(out_names, out_seqs)`

    - :obj:`out_names`: list of aligned protein names
    - :obj:`out_seqs`: list of corresponding protein sequences
...
def run_mmseqs2_modes(
  ...
  Parameters:
    ...
    mode: Mode to run the MSA generation in. Can be "unpaired", "paired", or "unpaired_paired"
